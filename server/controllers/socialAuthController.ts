...
import { StrictRootFilterOperators } from "mongoose";
...
const getOrCreateZernioProfile = async (user: any): Promise<string> => {
  try {
    ...
    const createResult = await zernio.profiles.createProfile({ body: { name: `${user.name || user.email}'s workspace` } as any, })
